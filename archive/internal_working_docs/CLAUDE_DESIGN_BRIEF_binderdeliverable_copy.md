...
- Funnel: **3,964 designs** AF3-screened (1-seed) → **152** cleared the refold gate → **39** finalists (10-seed), across 4 targets. (Of the 152 selected into the refold tier, 150 returned a 3-seed score; 2 4-1BB designs were selected but did not return a refold score. This is a separate, refold-stage gap — distinct from the 10 designs dropped at the 1-seed screening stage that left that stage at 99.75% and never entered the funnel. Use **152** as the funnel number; it matches T1 and the funnel figure.)
